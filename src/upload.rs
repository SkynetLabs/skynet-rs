use crate::{util::make_uri, SkynetClient, SkynetError::*, SkynetResult, URI_SKYNET_PREFIX};
use hyper::{body, Request};
use mime::Mime;
use serde::Deserialize;
use std::{collections::HashMap, fs, io::Write, path::Path, str};
...
pub async fn upload_data(
    ...
    let (fieldname, filename) = if data.len() == 1 && opt.custom_dirname.is_none() {
        (opt.portal_file_fieldname.clone(), "".to_string())
    } else {
        if let Some(ref custom_dirname) = opt.custom_dirname {
            (
                opt.portal_directory_file_fieldname.clone(),
    ...
    let boundary = TextNonce::sized(68)
        .map_err(TextNonceError)?
        .into_string()
        .into_bytes();
    ...
    let mut req = req.uri(uri).header("Content-Type", content_type);
    ...
    let res: UploadResponse =
        serde_json::from_str(body_str).map_err(|_| PortalResponse(body_str.to_string()))?;
