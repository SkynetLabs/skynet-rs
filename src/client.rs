use crate::{
    download, encryption, registry, upload, util::DEFAULT_PORTAL_URL, DownloadOptions, KeyPair,
    Metadata, MetadataOptions, Skykey, SkykeyOptions, SkynetResult, UploadOptions,
};
...
impl SkynetClient {
    ...
    pub async fn add_skykey(&self, skykey: &str, opt: SkykeyOptions) -> SkynetResult<()> {
    ...
    pub async fn get_skykey_by_name(&self, name: &str, opt: SkykeyOptions) -> SkynetResult<Skykey> {
        encryption::get_skykey_by_name(self, name, opt).await
    }

    pub async fn get_skykey_by_id(&self, id: &str, opt: SkykeyOptions) -> SkynetResult<Skykey> {
