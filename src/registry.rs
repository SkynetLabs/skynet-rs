use crate::{util::make_uri, SkynetClient, SkynetError::*, SkynetResult};
use crypto::{blake2b::Blake2b, digest::Digest, ed25519};
...
fn hash_registry_entry(entry: &RegistryEntry, hashed_data_key_hex: bool) -> Vec<u8> {
    ...
    Digest::input(
        &mut hasher,
        hash_data_key(&entry.data_key, hashed_data_key_hex).as_bytes(),
    );
...
pub async fn get_registry_entry(
    ...
    query.insert(
        "publickey".into(),
        format!("ed25519:{}", public_key.encode_hex::<String>()),
    );
    query.insert(
        "datakey".into(),
        hash_data_key(data_key, opt.hashed_data_key_hex),
    );
